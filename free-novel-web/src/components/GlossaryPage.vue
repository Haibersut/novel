              ...
              <template v-if="term.statue === 0">
                ...
                <td class="px-4 py-3 text-sm table-cell-content">
                  <div
                      ...
                      @blur="handleInlineEdit(term.id, $event)"
...
export default {
  ...
  methods: {
    ...
    async findOriginalTerms(page) {
      ...
      try {
        const response = await service.get(`/api/terminologies/findAllByNovelIdAndStatue/${this.$route.params.id}/${page - 1}/${this.itemsPerPage}`);
        this.termsCache = {
          ...this.termsCache,
    ...
    async searchTerms(page) {
      ...
      try {
        const response = await service.get(`/api/terminologies/searchAllByNovelIdAndStatue/${this.$route.params.id}/${page - 1}/${this.itemsPerPage}`, {
     ...
     * It updates the main `termsData` array first, then the `searchResults` if applicable.
     * @param {number} termId - The ID of the term being edited.
     * @param {Event} event - The blur event from the contenteditable div.
     */
    handleInlineEdit(termId, event) {
      const newValue = event.target.textContent.trim();
      const term = this.termsData.find(t => t.id === termId);
      if (!term) return;

      // Only update if the value has changed
      const originalValue = term.targetName;
      if (newValue === originalValue || newValue === term.modifyName) {
        return;
      }
      ...
      if (termIndex !== -1) {
        if (this.termsData[termIndex].statue === 0) {
          this.termsData[termIndex].modifyName = newValue;
          this.termsData[termIndex].statue = 2;
        } else if (this.termsData[termIndex].statue === 2) {
          this.termsData[termIndex].modifyName = newValue;
        }
      }
    ...
    async handleSync() {
      ...
      try {
        const modifiedTerms = this.filteredToModifyTerms.map(({ id, novelId, modifyName }) => ({ id, novelId, modifyName }));
        const deletedTermIds = this.filteredToDeleteTerms.map(item => item.id);

        await service.post("/api/terminologies/processTerminology", {
